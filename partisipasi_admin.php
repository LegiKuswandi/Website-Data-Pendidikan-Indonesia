              ...
              <div class="col-12">
                  <div class="card mt-1">
                      <div class="card-header">
                          <h3 class="card-title">Data Partisipasi Pendidikan</h3>
                      </div>
                      <div class="ms-3 mt-3">
                        <div class="form-outline d-flex justify-content-end">
                            <select class="me-4" aria-label="Category" id="getBH" name="getBH" required>
                                ...
                                <a class="me-4" href="tambah_partisipasi.php"><button type="button" class="btn btn-success">Tambah Data</button></a>
                          </div>
                      </div>
                      <!-- /.card-header -->
                      <div class="card-body">
                              ...
                              <thead>
                                  ...
                                  <?php
                                    foreach ($listps as $ps) {
                                        $listps = readpartisi($ps['id_partisipasi'])
                                        ?>
                                        <tr>
                                            <td><?=$ps['nama_provinsi']?></td>
                                            <td><?=$ps['angka_partisipasi_sekolah']?>%</td>
                                            <td><?=$ps['angka_partisipasi_kasar']?>%</td>
                                            <td><?=$ps['angka_partisipasi_murni']?>%</td>
                                            <td><?=$ps['tahun_partisipasi']?></td>
                                            <td><?=$ps['jenjang']?></td>
                                            <td>
                                                <a href="edit_partisipasi.php?id_partisipasi=<?=$ps['id_partisipasi']?>" class="link-warning"><i class="bi bi-pencil-square">Edit</i></a>
                                                <a href="delete_partisi.php?id=<?=$ps['id_partisipasi']?>" onclick="return confirm('Yakin Hapus?')"><i class="bi bi-trash3">Delete</i></a>
                                            </td>
                                        </tr>
                                        <?php
                                      }
