<?php
  ...
  if (isset($_GET['id_partisipasi'])) {
    $id = ($_GET["id_partisipasi"]);
    $result = readQuery('tpartisipasi', 'id_partisipasi', $id);
    $data = mysqli_fetch_assoc($result);
       if (!count($data)) {
            echo "<script>alert('Data tidak ditemukan pada database');window.location='partisipasi_admin.php';</script>";
        }
    } else {
        echo "<script>alert('Masukkan data id.');window.location='partisipasi_admin.php';</script>";
    }   

    if (isset($_POST['btn-edit-ps'])) {
        // jalankan query tambah record baru
        $isAddSucceed = updatepartisipasi($_POST, $_FILES);
        if ($isAddSucceed > 0) {
            // jika penambahan sukses, tampilkan alert
            echo "
            <script>
                alert('Data Berhasil di update');
                document.location.href = 'partisipasi_admin.php';
            </script>
        ";
        } else {
            echo "
            ...
            <div class="gradient-custom-1 h-100">
                ...
                <div class="container">
                    <div class="section-header">
                        <br>
                        <p>Edit<span> Data Partisipasi </span></p>
                    </div>
                    <form action="#" method="POST" id="form-add" enctype="multipart/form-data">
                        <input type="hidden" name="id" id="id" value="<?=$data['id_partisipasi']?>">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="persentase" class="form-label">Persentase APS</label>
                            <input class="form-control" type="number" step="any" id="aps" name="aps"  value="<?=$data['angka_partisipasi_sekolah']?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="persentase" class="form-label">Persentase APK</label>
                            <input class="form-control" type="number" step="any" id="apk" name="apk" value="<?=$data['angka_partisipasi_kasar']?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="persentase" class="form-label">Persentase APM</label>
                            <input class="form-control" type="number" step="any" id="apm" name="apm" value="<?=$data['angka_partisipasi_murni']?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="tahun" class="form-label">Tahun</label>
                            <input class="form-control" id="tahun" name="tahun" value="<?=$data['tahun_partisipasi']?>" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" aria-label="Category" id="jenjang" name="jenjang" required>
                                ...
                                <option value="<?=$listJenjang['id_jenjang']?>" <?php 
                                  if ($listJenjang['id_jenjang'] == $data['id_rentang']) {?>
                                  selected
                                  <?php 
                                  }
                                  ?>>
                        ...
                        <button type="submit" name="btn-edit-ps" id="btn-edit-ps" form="form-add" class="btn btn-success">Submit</button>
                        <a href="partisipasi_admin.php"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></a>
                    </form>
                    </div>
                </div>
                </div>
