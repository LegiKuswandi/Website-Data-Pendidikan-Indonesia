<?php
  ...
  if (isset($_GET['id_lembaga'])) {
    $id = ($_GET["id_lembaga"]);
    $result = readQuery('tlembaga', 'id_lembaga', $id);
    $data = mysqli_fetch_assoc($result);
    $tfs = readtfs($data['id_lembaga']);
       if (!count($data)) {
          echo "<script>alert('Data tidak ditemukan pada database');window.location='lembaga_admin.php';</script>";
       }
  } else {
    echo "<script>alert('Masukkan data id.');window.location='lembaga_admin.php';</script>";
  }   

  if (isset($_POST['btn-edit-lembaga'])) {
    
    $listFasilitas = $_POST['listFasilitas'];
    // jalankan query tambah record baru
    $isAddSucceed = updatelembaga($_POST, $_FILES, $listFasilitas);
    if ($isAddSucceed > 0) {
    ...
    <section id="add-menu" class="about">
      ...
      <div class="container" data-aos="fade-up">
            <div class="gradient-custom-1 h-100">
                <div style="margin-left: 260px;" class="mask d-flex align-items-center h-100">
                <div class="container">
                    <div class="section-header">
                        <br>
                        <p>Edit<span> Data Lembaga Pendidikan </span>Baru</p>
                    </div>
                    <form action="#" method="POST" id="form-add" enctype="multipart/form-data">
                        <input type="hidden" name="id" id="id" value="<?=$data['id_lembaga']?>">
                        <div class="mb-2">
                                <label class="form-label required">Nama Lembaga</label>
                                <input type="text" class="form-control" id="nama_lembaga" name="nama_lembaga" value="<?=$data['nama_lembaga']?>">
                            </div>
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="number" class="form-control" id="pengajar" name="pengajar" value="<?=$data['jumlah_pengajar']?>">
                            </div>
                            <div class="mb-2">
                                <label class="form-label required">Jumlah Murid</label>
                                <input type="number" class="form-control" id="murid" name="murid" value="<?=$data['jumlah_murid']?>">
                            </div>
                            <div class="mb-2">
                            <label for="jenjang">Jenjang</label>
                            <select class="form-select" aria-label="Category" id="jenjang" name="jenjang" required>
                                <?php
                                    while ($listJenjang = mysqli_fetch_assoc($listjenjang))
                                    {
                                ?>
                                <option value="<?=$listJenjang['id_jenjang']?>" <?php 
                                  if ($listJenjang['id_jenjang'] == $data['id_jenjang']) {?>
                                  selected
                                  <?php 
                                  }
                                  ?>>
                        ...
                        <select class="form-select" aria-label="Category" id="provinsi" name="provinsi" required>
                            <?php
                                    while ($listProv = mysqli_fetch_assoc($listprovinsi))
                                    {
                                ?>
                        ...
                        <label for="fasilitas">Pilih Fasilitas</label>
                            <div class="container">
                                <div class="row g-0">
                                    <?php
                                    while($listFasilitas = mysqli_fetch_assoc($listfasilitas))
                                    {
                                      ?>
                                      <div class="col-md-3">
                                          <input type="checkbox" name="listFasilitas[]"  value="<?=$listFasilitas['id_fasilitas']?>" 
                                            <?php
                                              foreach ($tfs as $listtfs ) {
                                                if ($listtfs['id_fasilitas'] == $listFasilitas['id_fasilitas']) {?>
                                                checked
                                                <?php
                                                }
                                              }
                                            ?>
                                          >
                                          <label><?=$listFasilitas['nama_fasilitas']?></label> 
                                      </div>
                                  <?php
                                  }
                                  ?>
                            ...
                            <button type="submit" name="btn-edit-lembaga" id="btn-edit-lembaga" form="form-add" class="btn btn-success">Submit</button>
                            <a href="lembaga_admin.php"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></a>
                    </form>
                    </div>
                </div>
                </div>
