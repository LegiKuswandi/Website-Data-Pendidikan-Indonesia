<?php
  ...
  if (isset($_GET['id_ebook'])) {
    $id = ($_GET["id_ebook"]);
    $result = readQuery('tebook', 'id_ebook', $id);
    $data = mysqli_fetch_assoc($result);
       if (!count($data)) {
            echo "<script>alert('Data tidak ditemukan pada database');window.location='e-book_admin.php';</script>";
        }
    } else {
        echo "<script>alert('Masukkan data id.');window.location='e-book_admin.php';</script>";
    }   

    if (isset($_POST['btn-edit-ebook'])) {
        // jalankan query tambah record baru
        $isAddSucceed = updateebook($_POST, $_FILES);
        if ($isAddSucceed > 0) {
            ...
            <div class="gradient-custom-1 h-100">
                ...
                <div class="container">
                    <div class="section-header">
                        <br>
                        <p>Edit<span> E-Book </span></p>
                    </div>
                    <form action="#" method="POST" id="form-add" enctype="multipart/form-data">
                        <input type="hidden" name="id" id="id" value="<?=$data['id_ebook']?>">
                        <div class="mb-3">
                            <label class="form-label required">Nama E-Book</label>
                            <input type="text" class="form-control" id="nama_ebook" name="nama_ebook" value="<?=$data['nama_ebook']?>" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="ebook" class="form-label">Abaikan Jika tidak mengganti file</label>
                            <input class="form-control" type="file" id="ebook" name="ebook">
                        </div>
                        <button type="submit" name="btn-edit-ebook" id="btn-edit-ebook" form="form-add" class="btn btn-success">Submit</button>
                        <a href="e-book_admin.php"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></a>
                    </form>
                    </div>
                </div>
                </div>
